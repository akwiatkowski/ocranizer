class Ocranizer::HtmlGenerator
  ...
  private def html_head(str)
    ...
    # custom
    str << ".calendar-day{width: #{cell_width}px;height: 80px;border: 1px dotted #888; color: #000; position: relative; padding: 12px 4px 4px 4px; }"
    str << ".calendar-day .day{position: absolute; top: 3px; left: 3px; font-size: 50%}"
  ...
  private def html_per_day(str, day : Time, month : Time)
    str << "<div class=\"day\">#{day.to_s("%Y-%m-%d")}</div>"
