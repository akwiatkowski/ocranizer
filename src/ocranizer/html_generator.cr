class Ocranizer::HtmlGenerator
  ...
  private def html_head(str)
    ...
    str << ".entity-list-item .entity-day{position: absolute; top: 25px; left: 0px; font-size: 150%; width: 80px; text-align: center} "
  ...
  private def html_per_entity(str, entity)
    ...
    if entity.time_from
      ...
      str << entity.time_from.not_nil!.time.to_s("%m-%d")
      str << "</div>"
